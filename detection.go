...
// detectReason determines if and why a file is auto-generated by reading it from the given filesystem.
// Two-phase detection: filename-based first (zero I/O), then content-based (reads file).
func detectReason(fsys fs.FS, filePath string, options map[FilterOption]bool) FilterReason {
	if reason := getFilenameBasedReason(filePath, options); reason != ReasonNotFiltered {
		return reason
	}
