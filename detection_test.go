...
func TestDetectorOptionReasonConsistency(t *testing.T) {
	...
	for _, d := range detectors {
		if !d.option.IsValid() {
			t.Errorf("detector option %q is not a valid FilterOption", d.option)
		}

		if !d.reason.IsValid() {
			t.Errorf("detector reason %q is not a valid FilterReason", d.reason)
		}

		if d.option.Reason() != d.reason {
			t.Errorf("detector{option=%q}.Reason() = %q, want %q",
				d.option, d.option.Reason(), d.reason)
		}
	}
}
