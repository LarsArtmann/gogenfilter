...
- **Theme system** — Dark mode default. Light mode via `data-theme="light"` attribute on `<html>` (unified with Starlight's convention). Toggle persists to `localStorage["starlight-theme"]` (old `"theme"` key read as backward-compat fallback). Initialize with `prefers-color-scheme` as fallback. CSS variables in `src/styles/global.css` under `:root[data-theme="light"]`.
...
- **Theme systems unified (2026-08-10)** — The landing page now uses the same `data-theme` attribute on `<html>` as Starlight docs (`data-theme="light"` / `data-theme="dark"`), and both share the `localStorage["starlight-theme"]` key. Previously the landing page used a `.light` CSS class + separate `localStorage["theme"]` key, causing theme preferences to not persist across landing↔docs navigation. The old `"theme"` key is read as a backward-compat fallback. Changes: `global.css` (`:root.light` → `:root[data-theme="light"]`), `theme-init.js` (sets `dataset.theme`), `header.js` (toggles `dataset.theme`, writes `starlight-theme`). The landing page toggle is two-state (light/dark); Starlight's docs toggle is three-state (light/dark/auto). When a user selects "auto" in docs then visits the landing page, the landing page resolves "auto" via `prefers-color-scheme`.
