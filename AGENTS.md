...
- **No `samber/oops` dependency** — The project uses its own branded error system, not `samber/oops`. The `erraudit` tool's "project enforces samber/oops" is a false assumption. Error types: `ProjectRootError`, `FilterConfigError`, `FileReadError`, `SQLCConfigError` — all implement `ErrorCoder` with sentinel errors for `errors.Is`.
