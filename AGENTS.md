...
- **CSP (Content Security Policy)** — Astro v6 built-in CSP (`security.csp` in `astro.config.mjs`). Astro auto-computes SHA-256 hashes for all inline scripts (including Starlight's ThemeProvider, ThemeSelect, Search, SidebarPersister, Tabs) and emits a `<meta http-equiv="content-security-policy">` tag with the hashes. CSP is NOT in Firebase headers — Firebase only sets non-CSP security headers (HSTS, X-Frame-Options, etc.). External scripts in `public/js/` are allowed via `'self'`. **Style hash stripping**: Astro also auto-hashes inline `<style>` blocks and adds those hashes to `style-src`, which causes `'unsafe-inline'` to be ignored (CSP Level 2 spec: hashes override `'unsafe-inline'`). Starlight uses inline `style=""` attributes extensively (SVG icons, theme select, TOC depth, dialogs). Post-build script `scripts/fix-csp.mjs` strips style hashes from all HTML files so `style-src 'self' 'unsafe-inline'` works without hash interference. Script hashes in `script-src` are preserved. If adding new inline scripts, Astro will automatically hash them at build time.
...
- **CSP via Astro meta tags + post-build style hash stripping** — Starlight injects inline scripts (ThemeProvider, ThemeSelect, Search, SidebarPersister, Tabs). Firebase `script-src 'self'` blocks these since static hosting can't generate nonces. Solution: Astro v6 built-in CSP (`security.csp`) auto-computes SHA-256 hashes at build time and emits `<meta>` tags. Firebase headers only set non-CSP security headers (HSTS, X-Frame-Options, etc.). However, Astro also auto-hashes inline `<style>` blocks into `style-src`, which makes `'unsafe-inline'` ineffective (CSP Level 2 spec). Post-build `scripts/fix-csp.mjs` strips style hashes so Starlight's inline `style=""` attributes work. Script hashes are preserved.
