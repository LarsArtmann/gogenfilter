...
- **Theme split-brain is intentional** — The landing page uses a `.light` class on `<html>` (custom JS in `public/js/theme-init.js` + `public/js/header.js`, persists to `localStorage` key `"theme"`). Starlight docs use their own `data-theme` attribute + toggle + persistence. Unifying would require overriding Starlight's theme system (breaks on update) or re-implementing the landing page in Starlight (heavy). Both default to `prefers-color-scheme` and sync `<meta name="theme-color">`, so the visual outcome is consistent; only the storage mechanism differs. This is an accepted trade-off, not a bug.
