...
The `Operation` field uses the `SQLCOperation` type (not raw `string`) to represent what was happening when the error occurred. Valid values: `OpSQLCFind`, `OpSQLCWalk`, `OpSQLCRead`, `OpSQLCCollect`, `OpSQLCParse`.
...
The `SQLCOperation` type represents what operation was being performed when a sqlc config error occurred.
...
| `OpSQLCWalk`    | `"walk"`               |
| `OpSQLCRead`    | `"read"`               |
| `OpSQLCCollect` | `"collect-output-dirs"` |
| `OpSQLCParse`   | `"parse"`              |

Has a `String() string` method. Used as the `Operation` field type on `SQLCConfigError`.
