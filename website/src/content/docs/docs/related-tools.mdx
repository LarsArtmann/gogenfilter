...
For intentionally partial code snippets, place a directive before the code block. The `<!-- skip-validate -->` HTML comment goes on the line above the code fence. Other options: `<!-- skip-md-validate -->`, `<!-- md-skip -->`, `<!-- no-validate -->`, `// skip-validate` (inside code), `//nolint` (inside code).
