...
var _ = ginkgo.Describe("gogenfilter extended", func() {
	...
	ginkgo.Describe("Combined include and exclude patterns", func() {
		...
		ginkgo.When("file matches neither", func() {
			ginkgo.It("gets outside-scope", func() {
				...
				gomega.Expect(stats.FilteredBy(gogenfilter.ReasonOutsideScope)).To(gomega.Equal(1))
			})
		})
	})
	...
	ginkgo.Describe("Metrics detail", func() {
		ginkgo.It("FilteredFiles returns a defensive copy", func() {
			opts, err := gogenfilter.WithFilterOptions(gogenfilter.FilterSQLC)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			...
			stats := filter.GetStats()
			files1 := stats.FilteredFiles(gogenfilter.ReasonSQLC)
			files2 := stats.FilteredFiles(gogenfilter.ReasonSQLC)
			gomega.Expect(files1).To(gomega.Equal(files2))
			gomega.Expect(files1).NotTo(gomega.BeNil())
		})

		ginkgo.It("disabled filter returns zero TotalFilesChecked", func() {
			filter, err := gogenfilter.NewFilter()
			gomega.Expect(err).NotTo(gomega.HaveOccurred())

			stats := filter.GetStats()
			gomega.Expect(int(stats.TotalFilesChecked)).To(gomega.Equal(0))
		})
	})
